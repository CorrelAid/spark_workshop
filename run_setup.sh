...
CONTAINER_NAME_OR_ID = jupyter/pyspark-notebook
...
if command -v docker &> /dev/null; then
    ...
    if docker images --format "{{.Repository}}:{{.Tag}}" | grep -q "$IMAGE_NAME"; then
        echo "$IMAGE_NAME has been pulled"
    else
   ...
   # Check if the container exists
    if docker ps -a -q --filter "name=$CONTAINER_NAME_OR_ID" 1>/dev/null; then
        # Check if the container is running
        if docker ps -q --filter "name=$CONTAINER_NAME_OR_ID" 1>/dev/null; then
            echo "Container $CONTAINER_NAME_OR_ID is already running."
        else
            # Start the container if it exists but is not running
            docker start $CONTAINER_NAME_OR_ID
            echo "Container $CONTAINER_NAME_OR_ID started."
        fi
    else
